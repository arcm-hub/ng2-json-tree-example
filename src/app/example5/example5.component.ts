...
export class Example5Component implements OnInit {
  ...
  onEvent(event: any) {
    ...
    } else if(event.type === "TEXT_INPUT_CREATED" && event.data.modelKey === 'price') {
        event.data.toolbar.append('span')
        .attr('class', 'ng2-json-tree-toolbar-button')
        .text('remove')
        .on('click', function(d, i) {
            event.data.d3Container.remove(); //Removing UI Element
            delete event.data.parentModel[event.data.modelKey]; // Deleting the actual data on the model
            console.log("CLICKED", event);
        });
    }
  }
